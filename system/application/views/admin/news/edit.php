    ...
    <form id="form" class="table_input" action="<?=$base_url;?>admin/cnews/edit/<?=$currentcatalogue->id;?>/<?=$object->id != '' ? $object->id : 0 ;?>" method="post" enctype="multipart/form-data">
        <table class="table_input">
            ...
            <input type="hidden" name="dir" value="<?=$object->dir;?>" >
 			<tr>
             	<td><label for="name">Chọn đại diện:</label></td>
                <td><input type="file" name="image" class="smallInput medium" /></td>
            </tr>
            <tr>
            	<td><label for="name">Thông tin SEO</label></td>
            	<td></td>
            </tr>
            <tr>
             	<td><label for="name">Title (SEO):</label></td>
                <td><input type="text" name="page_title" class="smallInput big" value="<?=$object->page_title;?>" /></td>
            </tr>
            <tr>
             	<td><label for="name">Keyword (SEO):</label></td>
                <td><input type="text" name="page_keyword" class="smallInput big" value="<?=$object->page_keyword;?>" /></td>
            </tr>
            <tr>
        ...
        <div id="container_language" style="margin-top:10px;">
            <?php foreach($sitelanguage as $row_lang):?>  
          	<div id="<?=$row_lang->short;?>">  
                <table class="table_input">
					<tr>
						<td width="120px;"><label for="name">Tiêu đề:<span style="color:red">*</span></label></td>
						<td width="640px;"><textarea style="width: 700px;" name="title_<?=$row_lang->short;?>" class="smallInput big" ><?=$object->{"title_".$row_lang->short};?></textarea></td>                            
					</tr>
					<tr>
						<td width="120;"><label for="name">Mô tả:<span style="color:red">*</span></label></td>
						<td><textarea style="width: 700px; height: 80px;" name="short_<?=$row_lang->short;?>" class="smallInput big" ><?=$object->{"short_".$row_lang->short};?></textarea></td>                            
					</tr>
                </table>   
